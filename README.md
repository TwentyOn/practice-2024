...
После запуска сервера взаимодействие с API происходит по таким же адресам, которые описанны в разделе "Локальный запуск"
